...
function Command_runCallbackInDir(dir,func) {
	let oldCwd = haxe_io_Path.addTrailingSlash(process.cwd());
	Command_cd(dir);
	let result = func();
	Command_cd(oldCwd);
	return result;
...
function Tests_make(target) {
	...
	let target4 = target;
	let tmp2 = function() {
		return Tests_buildDemos(target4);
	};
	let target5 = target;
	let tmp3 = function() {
...
function Tests_buildDemos(target,demos,args) {
	let tmp = args;
	args = tmp != null ? tmp : [];
	if(demos != null) {
	...
	return Flixel_buildProjects(target,demos.concat(args));
}
